...
`summit-ui-canvas-trail` is centered on frontend apps work in Lua. The technical target is: Develop a Lua command-oriented project for canvas scenarios with fixture event logs, golden state snapshots, and no credentials or hosted services.
...
The score is deterministic. The current threshold is 171. Risk has a penalty of 5, latency has a penalty of 4, and weight contributes a bonus of 2.
